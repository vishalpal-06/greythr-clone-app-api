...
def create_payslips(
    payslip_in: PayslipCreate, db: db_dependency, user: user_dependency
):
    _require_admin(user)
...
def get_payslips_by_employeeid(
...
def get_payslips_by_month_endpoint(
...
@router.get(
    "/employee/{employee_id}/month/{year}/{month}", response_model=PayslipResponse
)
def get_payslips_by_month_and_employeeid(
    ...
    return get_payslip_by_employee_and_month(
        db=db, employee_id=employee_id, year=year, month=month
    )
