...
def create_new_role_endpoint(role_data: RoleCreate, db: db_dependency, user: user_dependency):
    return create_role(db=db, role_name=role_data.role, user=user)


# Update by ID
@router.put("/id/{role_id}", response_model=RoleResponse)
def rename_role_by_id_endpoint(role_id: int, new_name: str, db: db_dependency, user: user_dependency):
...
def rename_role_by_name_endpoint(current_name: str, new_name: str, db: db_dependency, user: user_dependency):
...
def remove_role_by_name_endpoint(role_name: str, db: db_dependency, user: user_dependency):
