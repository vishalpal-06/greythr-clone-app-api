...
def get_all_employee_salary_by_year(year: int, db: db_dependency, user: user_dependency):
    _require_admin(user)
    return get_salaries_by_year(db, year=year)  # raises 404 if empty


@router.get("/employee/{employee_id}/year/{year}", response_model=salary_schema.SalaryResponse)
def get_specific_employee_salary_by_year(employee_id: int, year: int, db: db_dependency, user: user_dependency):
    _require_admin(user)
    return get_salary_by_employee_and_year(db, employee_id=employee_id, year=year)


@router.get("/employee/{employee_id}/salaries", response_model=List[salary_schema.SalaryResponse])
def get_employee_all_salaries(employee_id: int, db: db_dependency, user: user_dependency):
    _require_admin(user)
...
def post_salary(salary_in: salary_schema.SalaryCreate, db: db_dependency, user: user_dependency):
    _require_admin(user)
