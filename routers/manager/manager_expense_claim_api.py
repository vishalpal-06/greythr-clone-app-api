...
from common.expense_claim import (
    get_manager_claim_by_id, get_manager_claims_by_status,
    get_manager_claims_by_month, get_manager_claims_by_employee_and_month,
...
def get_by_id_where_manager_current_user(claim_id: int, db: db_dependency, user: user_dependency):
...
def get_all_expense_by_month_where_manager_current_user(year: int, month: int, db: db_dependency, user: user_dependency):
...
def get_all_expense_by_status_where_manager_current_user(status: Status, db: db_dependency, user: user_dependency):
...
def get_expense_by_employeeid_and_month_where_manager_currentuser(
    ...
    return get_manager_claims_by_employee_and_month(db=db, emp_id=emp_id, year=year, month=month, user=user)


@router.put("/{claim_id}/status", response_model=ExpenseClaimResponse)
def update_exp_status_where_manager_currentuser(
    claim_id: int, update: ExpenseClaimStatusUpdate, db: db_dependency, user: user_dependency
):
