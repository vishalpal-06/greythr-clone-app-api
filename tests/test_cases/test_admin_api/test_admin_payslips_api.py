...
def test_admin_get_my_payslips_by_year_and_month_success(client, admin_user, read_json):
    response = client.get(
        "/user/my/payslips/month/2025/1",
        headers={"Authorization": f"Bearer {admin_user}"},
    )
    expected = read_json(
        "expected_responses/admin/payslips/get_my_payslips_by_year_and_month_admin.json"
    )
    assert response.status_code == 200
    assert response.json() == expected
...
def test_admin_admin_access_create_payslips_success(client, admin_user, read_json):
    ...
    expected = read_json("expected_responses/admin/payslips/create_payslip_admin.json")
    assert response.status_code == 201
    assert response.json() == expected


def test_admin_admin_access_create_payslips_conflict(client, admin_user, read_json):
    payload = {
        "basic_amount": 80000,
        "hra": 40000,
...
def test_admin_admin_access_get_payslips_by_id_not_found(client, admin_user, read_json):
    response = client.get(
...
def test_admin_admin_access_get_payslips_by_year_and_month_success(
    ...
    expected = read_json(
        "expected_responses/admin/payslips/get_payslips_by_year_and_month_admin.json"
    )
    assert response.status_code == 200
    assert response.json() == expected
...
def test_admin_admin_access_get_payslips_by_empid_year_and_month_success(
    client, admin_user, read_json
):
    response = client.get(
    ...
    expected = read_json(
        "expected_responses/admin/payslips/get_payslips_by_empid_year_and_month_admin.json"
    )
    assert response.status_code == 200
    assert response.json() == expected
...
def test_admin_admin_access_delete_payslips_by_empid_year_and_month_emp_not_found(
    client, admin_user
):
    response = client.delete(
