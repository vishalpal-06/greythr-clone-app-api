...
def test_user_admin_access_get_payslips_by_year_and_month_forbidden(client, user_A1, read_json):
    response = client.get(
...
def test_user_admin_access_delete_payslips_by_id_forbidden(client, user_A1, read_json):
    response = client.delete("/admin/payslips/1", headers={"Authorization": f"Bearer {user_A1}"})
    assert response.status_code == 403
    assert response.json() == {"detail": "Admin privileges required"}
