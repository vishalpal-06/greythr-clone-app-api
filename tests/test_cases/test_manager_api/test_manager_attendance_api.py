...
def test_manager_manager_access_get_subordinate_attendance_by_date_success(client, manager_A):
    response = client.get(
...
def test_manager_manager_access_get_subordinate_attendance_by_empid_and_date_success(client, manager_A):
    response = client.get(
        "manager/attendance/employee/5/date/2023-04-02",
        headers={"Authorization": f"Bearer {manager_A}"},
    )
    expected = read_json("get_subordinate_attendance_by_employee_and_date_managerA.json")
    assert response.status_code == 200
    assert response.json() == expected


def test_manager_manager_access_get_subordinate_attendance_by_empid_and_date_nonsubordinate_empty(client, manager_A):
    response = client.get(
