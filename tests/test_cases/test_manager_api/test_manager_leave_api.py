...
def test_manager_admin_access_get_employee_leaves_by_empid_forbidden(client, manager_A, read_json):
    response = client.get(
...
def test_manager_admin_access_delete_leave_by_id_forbidden(client, manager_A, read_json):
    response = client.delete("admin/leaves/1", headers={"Authorization": f"Bearer {manager_A}"})
