...
def test_manager_get_my_all_leave_success(client, manager_A, read_json):
    ...
    expected = read_json(
        "expected_responses/manager/leave/get_all_my_leave_manager_A.json"
    )
...
def test_manager_get_my_leave_by_year_success(client, manager_A, read_json):
    ...
    expected = read_json(
        "expected_responses/manager/leave/get_my_leave_by_year_manager_A.json"
    )
    assert response.status_code == 200
    assert response.json() == expected
...
def test_manager_admin_access_get_employee_leaves_by_empid_forbidden(
...
def test_manager_admin_access_create_leave_forbidden(client, manager_A, read_json):
    payloads = {
        "assign_year": 2000,
        "casual_leave": 0,
        "plan_leave": 0,
...
def test_manager_admin_access_delete_leave_by_id_forbidden(
    client, manager_A, read_json
):
    response = client.delete(
