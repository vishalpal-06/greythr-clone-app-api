...
def test_manager_admin_access_delete_role_by_id_forbidden(client, manager_A, read_json):
    response = client.delete("/admin/roles/id/1", headers={"Authorization": f"Bearer {manager_A}"})
    assert response.status_code == 403
    assert response.json() == {"detail": "Admin privileges required"}


def test_manager_admin_access_update_role_by_name_forbidden(client, manager_A, read_json):
    response = client.put(
...
def test_manager_admin_access_delete_role_by_name_forbidden(client, manager_A, read_json):
