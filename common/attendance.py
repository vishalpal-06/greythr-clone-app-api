...
def get_my_attendance_by_date(db: db_dependency, user: user_dependency, punch_date: date):
...
def get_manager_team_attendance_by_date(
    ...
    has_subordinates = db.query(Employee).filter(Employee.fk_manager_id == user["id"]).first()
    ...
    managed_ids = [
        e.employee_id
        for e in db.query(Employee.employee_id).filter(Employee.fk_manager_id == user["id"]).all()
    ]
...
def get_manager_subordinate_attendance_by_date(
    ...
    employee = (
        db.query(Employee)
        .filter(Employee.employee_id == employee_id, Employee.fk_manager_id == user["id"])
        .first()
    )
