...
from schema.regularization_schema import RegularizationCreate, RegularizationStatusUpdate, Status
...
def get_my_regularizations(db: Session, user: dict) -> List[Regularization]:
    return db.query(Regularization)\
...
def get_my_regularizations_by_month(year: int, month: int, db: Session, user: dict) -> List[Regularization]:
    return db.query(Regularization)\
           .filter(
...
def admin_update_status(reg_id: int, status_update: RegularizationStatusUpdate, db: Session) -> Regularization:
    reg = _get_regularization_or_404(db, reg_id)
...
def get_manager_regularization_by_id(reg_id: int, db: Session, user: dict) -> Regularization:
    ...
    if reg.fk_manager_id != user["id"]:
        raise HTTPException(status_code=403, detail="Regularization not found under your management")
    return reg


def get_manager_regularizations_for_employee(employee_id: int, db: Session, user: dict) -> List[Regularization]:
...
def manager_update_regularization_status(reg_id: int, status_update: RegularizationStatusUpdate, db: Session, user: dict) -> Regularization:
