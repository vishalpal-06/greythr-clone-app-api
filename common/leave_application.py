...
from schema.leave_application_schema import LeaveApplicationCreate, LeaveApplicationStatusUpdate, Status
...
def create_leave_application(db: Session, app_in: LeaveApplicationCreate, user: dict) -> LeaveApplication:
...
def get_my_applications_by_status(db: Session, user: dict, status: Status) -> List[LeaveApplication]:
    return db.query(LeaveApplication)\
           .filter(LeaveApplication.fk_employee_id == user["id"],
...
def get_my_applications_by_month(db: Session, year: int, month: int, user: dict) -> List[LeaveApplication]:
    return db.query(LeaveApplication)\
           .filter(
...
def get_all_by_employee_admin(db: Session, emp_id: int) -> List[LeaveApplication]:
    apps = db.query(LeaveApplication).filter(LeaveApplication.fk_employee_id == emp_id)\
...
def admin_update_status(db: Session, app_id: int, update: LeaveApplicationStatusUpdate) -> LeaveApplication:
    app = _get_leave_app_or_404(db, app_id)
...
def get_manager_application_by_id(db: Session, app_id: int, user: dict) -> LeaveApplication:
    ...
    if app.fk_manager_id != user["id"]:
        raise HTTPException(status_code=403, detail="Leave Application not found under your management")
    return app


def get_manager_applications_by_status(db: Session, status: Status, user: dict) -> List[LeaveApplication]:
    return db.query(LeaveApplication)\
           .filter(LeaveApplication.fk_manager_id == user["id"],
                   LeaveApplication.leave_status == status)\
           .order_by(LeaveApplication.from_date)\
           .all()


def get_manager_applications_by_month(db: Session, year: int, month: int, user: dict) -> List[LeaveApplication]:
    return db.query(LeaveApplication)\
           .filter(
...
def get_manager_applications_by_employee(db: Session, emp_id: int, user: dict) -> List[LeaveApplication]:
...
def manager_update_status(db: Session, app_id: int, update: LeaveApplicationStatusUpdate, user: dict) -> LeaveApplication:
